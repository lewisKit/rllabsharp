...
from sandbox.rocky.tf.launchers.launcher_stub_utils import get_env, get_policy, get_baseline, get_qf, get_es, get_algo, get_pf
...
def set_experiment(mode="local", keys=None, params=dict()):
    ...
    algo = get_algo(n_itr=n_itr, env=env, policy=policy, baseline=baseline,
            qf=qf, es=es,pf=pf, max_path_length=max_path_length, **flags)
    return algo, dict(
