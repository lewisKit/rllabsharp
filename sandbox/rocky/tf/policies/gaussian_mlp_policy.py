...
class GaussianMLPPolicy(StochasticPolicy, LayersPowered, Serializable):
    ...
    def get_grad_info_sym(self, obs_var, action_var):
        # check if need deterministic=True parameters
        dist_info = self.dist_info_sym(obs_var)
        mean_var = dist_info['mean']
        log_std_var = dist_info['log_std']

        variance = (tf.exp(2. * log_std_var))
        # \log pi gradient w.r.t mu
        logpi_dmu = - (action_var - mean_var) / variance

        logpi_dvar = -.5 * (1 / variance - 
                tf.square(action_var - mean_var) / tf.square(variance))

        return dict(logpi_dmu=logpi_dmu, logpi_dvar=logpi_dvar), dist_info
